<script lang="ts">
	...
	const jsonData = writable<Record<string, any>[]>([]);
	...
	function parseJsonFile(file: File) {
		...
		reader.onload = (event) => {
			...
			try {
				...
				jsonData.update((current) => [...current, json]);
			} catch (error) {
	...
	function updateJsonData(index: number, key: string, value: any) {
...
>
	...
	<!-- Expansion Panels for Dataset Schema -->
