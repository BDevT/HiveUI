...
<div class="flex flex-col items-center justify-start min-h-screen bg-neutral p-4 text-neutral-content w-full">
	...
	<div bind:this={dropzone} class="w-full max-w-screen-xl p-6 border-4 border-dashed border-surface-content bg-surface-100 rounded-md text-center cursor-pointer hover:border-primary hover:bg-surface-200 transition">
		<p class="mb-4">Drag & Drop your files here</p>
		<button on:click={triggerFileInput} class="px-4 py-2 bg-primary text-neutral-content rounded hover:bg-primary-dark transition"> Or select files </button>
		<input bind:this={fileInput} type="file" multiple class="hidden" on:change={handleFileInputChange} />
	</div>
			...
			<ExpansionPanel>
				...
				<div class="p-4 grid grid-cols-2 gap-4">
					<Field label="Source Folder">
						<Input placeholder="Data source folder path" bind:value={currentMetaData.sourceFolder} />
					</Field>
					<Field label="Size">
						<Input placeholder="Size in bytes" type="number" bind:value={currentMetaData.size} />
					</Field>
					<Field label="Packed Size">
						<Input placeholder="Packed size in bytes" type="number" bind:value={currentMetaData.packedSize} />
					</Field>
			...
			<ExpansionPanel open={true}>
				...
				<div class="p-4 grid gap-4">
					<div class="grid grid-cols-2 gap-4">
						...
						<Field label="Physics">
							<Input placeholder="Physics related information" bind:value={currentMetaData.physics} />
						</Field>
							...
							{#each currentMetaData['thermocouples'] as _, index}
								...
								>
									<div slot="trigger" class="flex-1 p-3">
										...
										<Button icon={mdiTrashCan} color="danger" on:click={() => removeThermocouple(index)} />
									</div>
									<div class="p-4 grid grid-cols-2 gap-4">
										<Field label="Status">
										...
										<Field label="Attachment">
											<Input placeholder="e.g., spot weld with inert gas shield" bind:value={_.attachment} />
										</Field>
										<Field label="TC Type">
										...
										<Field label="Circle Diameter">
											<Input placeholder="Diameter in mm" type="number" bind:value={_.circleDiameter} />
										</Field>
								...
								>
									...
									<div class="p-4 grid grid-cols-2 gap-4">
										...
										<Field label="Image Scale">
											<Input placeholder="Scale in mm/pixel" type="number" bind:value={_.imageScale} />
										</Field>
										<Field label="Stereo Angle">
											<Input placeholder="Angle in degrees" type="number" bind:value={_.stereoAngle} />
										</Field>
										<Field label="Stand Off Distance">
											<Input placeholder="Distance in meters" type="number" bind:value={_.standOffDistance} />
										</Field>
										<Field label="Image Acquisition Rate">
											<Input placeholder="Rate in Hz" type="number" bind:value={_.imageAcquisitionRate} />
										</Field>
										<Field label="Image Noise">
											<Input placeholder="Noise percentage" type="number" bind:value={_.imageNoise} />
										</Field>
										...
										<Field label="Approx Feature Size">
											<Input placeholder="Size in microns" type="number" bind:value={_.approxFeatureSize} />
										</Field>
										<Field label="Lens Model">
											<Input placeholder="Lens model name" bind:value={_.lensModel} />
										</Field>
										<Field label="Lens Focal Length">
											<Input placeholder="Focal length in mm" type="number" bind:value={_.lensFocalLength} />
										</Field>
										<Field label="Lens Aperture">
											<Input placeholder="e.g., f/8" bind:value={_.lensAperture} />
										</Field>
										<Field label="Lens Filters">
											<Input placeholder="e.g., Edmund optics blue filter" bind:value={_.lensFilters} />
										</Field>
										...
										<Field label="Cal Target Spacing">
											<Input placeholder="Spacing in mm" type="number" bind:value={_.calTargetSpacing} />
										</Field>
									</div>
								</ExpansionPanel>
							{/each}
	...
	<div class="button-group w-full max-w-screen-xl">
		<div class="left-buttons">
			<button class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" on:click={exportForm}>Export</button>
		</div>
		<div class="right-buttons">
			<button class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600" on:click={resetForm}>Reset</button>
			<button class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" on:click={submitForm}>Submit</button>
		</div>
	</div>
	{:else}
